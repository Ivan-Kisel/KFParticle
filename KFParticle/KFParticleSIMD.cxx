...
void KFParticleSIMD::Create(KFPEmcCluster& track, uint_v& index, const KFParticleSIMD& vertexGuess)
{
  ...
  J[2][2] = 1.f;
  J[6][3] = 1.f;
  J[3][0] = fP[6] / dl * (1.f - dx * dx / dl2);
  J[3][1] = -dx * dy * fP[6] / (dl * dl2);
  J[3][2] = -dx * dz * fP[6] / (dl * dl2);
  J[3][3] = dx / dl;
  J[4][0] = -dx * dy * fP[6] / (dl * dl2);
  J[4][1] = fP[6] / dl * (1.f - dy * dy / dl2);
  J[4][2] = -dy * dz * fP[6] / (dl * dl2);
  J[4][3] = dy / dl;
  J[5][0] = -dx * dz * fP[6] / (dl * dl2);
  J[5][1] = -dy * dz * fP[6] / (dl * dl2);
  J[5][2] = fP[6] / dl * (1.f - dz * dz / dl2);
  J[5][3] = dz / dl;
...
float_v KFParticleSIMD::GetDeviationFromParticleXY(const KFParticleSIMD& p) const
{
  ...
  return ((mC1[0] * d[0] + mC1[1] * d[1]) * d[0] + (mC1[1] * d[0] + mC1[2] * d[1]) * d[1]);
}
...
float_v KFParticleSIMD::GetDeviationFromVertexXY(const float_v vtx[], const float_v Cv[]) const
{
  ...
  return ((mC[0] * d[0] + mC[1] * d[1]) * d[0] + (mC[1] * d[0] + mC[2] * d[1]) * d[1]);
}
