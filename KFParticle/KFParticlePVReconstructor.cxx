...
void KFParticlePVReconstructor::Init(KFPTrackVector* tracks, int nParticles)
{
  ...
  for (int iTr = 0; iTr < fNParticles; iTr++) {
    ...
    for (int iC = 0; iC < 3; iC++) {
      if (!(fParticles[0].Covariance(iC, iC) == fParticles[0].Covariance(iC, iC)))
        continue;
  ...
  for (int iP = 0; iP < fNParticles; iP++) {
    ...
    fWeight[iP] = fParticles[iP].CovarianceMatrix()[0] + fParticles[iP].CovarianceMatrix()[2] + fParticles[iP].CovarianceMatrix()[5];
    ...
    if ((fParticles[iP].X() * fParticles[iP].X() + fParticles[iP].Y() * fParticles[iP].Y()) > 100.f)
      fWeight[iP] = -100.f;
  }
...
void KFParticlePVReconstructor::FindPrimaryClusters(int cutNDF)
{
  ...
  while (nNotUsedTracks > 0) {
    ...
    if (cluster.fTracks.size() < 2)
      continue;
    if ((cluster.fTracks.size() < 3) && (fNParticles > 3))
      continue;
