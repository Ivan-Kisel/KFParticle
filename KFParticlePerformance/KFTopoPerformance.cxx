...
KFTopoPerformance::KFTopoPerformance() : KFParticlePerformanceBase(), fTopoReconstructor(0), fPrimVertices(0), fMCTrackToMCPVMatch(0), fPVPurity(0), fNCorrectPVTracks(0), fTrackMatch(0), vMCTracks(0), vMCParticles(0), fNeutralIndex(0), MCtoRParticleId(0), RtoMCParticleId(0), MCtoRPVId(0), RtoMCPVId(0), fPrintEffFrequency(1), fCentralityBin(-1), fCentralityWeight(0.f)
{
}
...
void KFTopoPerformance::GetMCParticles()
{
  ...
  mmMotherPDG[0] = 3112;
  mmChargedDaughterPDG[0] = 211;
  mmNeutralDaughterPDG[0] = 2112;
  mmMotherPDG[1] = 3222;
  mmChargedDaughterPDG[1] = 211;
  mmNeutralDaughterPDG[1] = 2112;
  mmMotherPDG[2] = 3312;
  mmChargedDaughterPDG[2] = 211;
  mmNeutralDaughterPDG[2] = 3122;
  mmMotherPDG[3] = 3334;
  mmChargedDaughterPDG[3] = 211;
  mmNeutralDaughterPDG[3] = 3322;
  mmMotherPDG[4] = 321;
  mmChargedDaughterPDG[4] = 211;
  mmNeutralDaughterPDG[4] = 111;
  mmMotherPDG[5] = 3334;
  mmChargedDaughterPDG[5] = 321;
  mmNeutralDaughterPDG[5] = 3122;
  mmMotherPDG[6] = 3222;
  mmChargedDaughterPDG[6] = 2212;
  mmNeutralDaughterPDG[6] = 111;

  newMotherPDG[0] = 7003112;
  newNeutralPDG[0] = 7002112;
  newMotherPDG[1] = 7003222;
  newNeutralPDG[1] = 8002112;
  newMotherPDG[2] = 7003312;
  newNeutralPDG[2] = 7003122;
  newMotherPDG[3] = 7003334;
  newNeutralPDG[3] = 7003322;
  newMotherPDG[4] = 9000321;
  newNeutralPDG[4] = 9000111;
  newMotherPDG[5] = 8003334;
  newNeutralPDG[5] = 8003122;
  newMotherPDG[6] = 8003222;
  newNeutralPDG[6] = 8000111;
  ...
  for (int iMC = 0; iMC < nMCParticles; iMC++) {
    if (abs(vMCParticles[iMC].GetPDG()) == 211 || abs(vMCParticles[iMC].GetPDG()) == 321) {
      ...
      if (muonIndex > -1) {
        ...
        if (vMCParticles[iMC].GetPDG() == -211)
          neutrinoPDG = -7000014;
        if (vMCParticles[iMC].GetPDG() == 321)
          neutrinoPDG = 8000014;
        if (vMCParticles[iMC].GetPDG() == -321)
          neutrinoPDG = -8000014;
...
void KFTopoPerformance::CheckMCParticleIsReconstructable(KFMCParticle& part)
{
  ...
  if (part.IsReconstructable(0))
    return;
  if (vMCTracks[part.GetMCTrackID()].IsOutOfDetector())
    return;
  ...
  //  mother particles
    ...
    for (int iPart = 0; iPart < fParteff.nParticles; iPart++) {
      if (part.GetPDG() == fParteff.partPDG[iPart]) {
        ...
        if (part.GetDaughterIds().size() != nDaughters)
          return;
        ...
        for (unsigned int iDMC = 0; iDMC < nDaughters; iDMC++)
          for (unsigned int iD = 0; iD < nDaughters; iD++)
            if (pdg[iD] == fParteff.partDaughterPdg[iPart][iDMC])
              isDaughterFound[iDMC] = 1;
    ...
    if (nD == 0)
      return; //TODO optimize for all species
    ...
    if (reco4 && iParticle >= KFPartEfficiencies::fFirstMissingMassParticleIndex &&
        iParticle <= KFPartEfficiencies::fLastMissingMassParticleIndex)
      part.SetAsReconstructable(3);
    if (reco5 && iParticle >= KFPartEfficiencies::fFirstMissingMassParticleIndex &&
        iParticle <= KFPartEfficiencies::fLastMissingMassParticleIndex)
      part.SetAsReconstructable(4);
  }
}
...
void KFTopoPerformance::FindReconstructableMCVertices()
{
  ...
  for (unsigned int iV = 0; iV < nMCVertices; iV++) {
    ...
    for (int iP = 0; iP < vert.NDaughterTracks(); iP++) {
      ...
      if (part.IsReconstructable(2))
        nReconstructableDaughters++;
      if (part.IsReconstructable(1))
        nMCReconstructableDaughters++;
    }

    if (nReconstructableDaughters >= 2)
      vert.SetReconstructable();
    else
      vert.SetUnReconstructable();

    if (nMCReconstructableDaughters >= 2)
      vert.SetMCReconstructable();
    else
      vert.SetMCUnReconstructable();
  }
}
...
void KFTopoPerformance::MatchParticles()
{
  ...
  for (unsigned int iRP = 0; iRP < fTopoReconstructor->GetParticles().size(); iRP++) {
    ...
    if ((abs(rPart.GetPDG()) == 7000014) || (abs(rPart.GetPDG()) == 8000014) || (abs(rPart.GetPDG()) == 7002112) ||
      ...
      if (chargedDaughter.GetMotherId() != mcMotherId)
        continue;
    ...
    //normal decays
      ...
      int mmId = -2; // MC id for rPart
      {
        ...
        if (!RtoMCParticleId[rdId].IsMatched())
          continue;
      ...
      for (; iD < NRDaughters; iD++) {
        ...
        if (isMissingMass) {
          ...
          if (mmId != vMCParticles[neutralDaughter.GetMotherId()].InitialParticleId())
            break;
          mmId = neutralDaughter.GetMotherId();
        }

        if (!(isMissingMass) && (vMCParticles[mdId].GetMotherId() != mmId))
          break;
      }
...
void KFTopoPerformance::CalculateEfficiency()
{
  ...
  for (int iP = 0; iP < NMP; ++iP) {
...

    {
      for (unsigned int iPType = 0; iPType < iParticle.size(); iPType++) {
        ...
        for (int iEff = 0; iEff < 3; iEff++) {
          if (!isReconstructable[iPType][iEff])
            continue;
          ...
          if (fStoreMCHistograms) {
            ...
            if (cT > -1.e10)
              hPartEfficiency[iPart][iEff][4]->Fill(cT, isReco[iPType]);
            if (decayLength > -1.e10)
              hPartEfficiency[iPart][iEff][5]->Fill(decayLength, isReco[iPType]);
            hPartEfficiency[iPart][iEff][3]->Fill(Z, isReco[iPType]);
  ...
  if (fNEvents % fPrintEffFrequency == 0) {
    ...
    std::cout << "ACCUMULATED STAT    : " << fNEvents << " EVENTS " << std::endl
              << std::endl;
    fParteff.PrintEff();
    std::cout << std::endl;
  }
                                               ...
                                               vector<int>* multiplicities)
{
  ...
  if (!(decays.empty()) && (iParticle < fParteff.fFirstStableParticleIndex || iParticle > fParteff.fLastStableParticleIndex))
    if (decays.find(TempPart.GetPDG()) == decays.end())
      return;
  ...
  Z = TempPart.GetZ();
#ifdef CBM
  if (Z >= 1. && iParticle >= 54 && iParticle <= 64)
    return;
#endif
  R = sqrt(X * X + Y * Y);
  ...
  if (!RtoMCParticleId[iP].IsMatchedWithPdg()) //background
  {
    if (!RtoMCParticleId[iP].IsMatched())
      iSet = 3; // for ghost particles - combinatorial background
    else
      iSet = 2; // for physical background
  }
  ...
  if (histoFit) {
    ...
    for (int iPar = 0; iPar < 7; iPar++) {
      ...
      if (fabs(errParam[iPar]) > 1.e-20)
        pull[iPar] = res[iPar] / errParam[iPar];
    }
  ...
  if (histoFitDaughtersQA) {
    for (int iD = 0; iD < mcPart.NDaughters(); ++iD) {
      ...
      if (!MCtoRParticleId[mcDaughterId].IsMatched())
        continue;
...
void KFTopoPerformance::FillHistos()
{
  ...
  for (int iPV = 0; iPV < fTopoReconstructor->NPrimaryVertices(); iPV++) {
    ...
    if (RtoMCPVId[iPV].IsMatched()) {
      ...
      for (int iPV2 = iPV + 1; iPV2 < fTopoReconstructor->NPrimaryVertices(); iPV2++) {
        if (!RtoMCPVId[iPV2].IsMatched())
          continue;
        int iCurrMCPV2 = RtoMCPVId[iPV2].GetBestMatch();
        if (iCurrMCPV != iCurrMCPV2)
          continue;
  ...
  for (unsigned int iP = 0; iP < fTopoReconstructor->GetParticles().size(); iP++) {
    ...
    if (!RtoMCParticleId[iP].IsMatchedWithPdg())
      continue; //ghost
    ...
    if (mcTrack.MotherId() > -1)
      continue; // select only PV tracks
...
void KFTopoPerformance::FillMCHistos()
{
  ...
  for (unsigned int iMCTrack = 0; iMCTrack < vMCTracks.size(); iMCTrack++) {
    ...
    if (vMCTracks[iMCTrack].MotherId() >= 0)
      continue;
