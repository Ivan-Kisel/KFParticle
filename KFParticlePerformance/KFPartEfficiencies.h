...
class KFEfficiencyParticleInfo
{
 public:
  ...
  KFEfficiencyParticleInfo(std::string name, std::string title, int pdg, float histoMin, float histoMax, float mass, float lifeTime, int charge, float massSigma) : fName(name), fTitle(title), fPDG(pdg), fHistoMin(histoMin), fHistoMax(histoMax), fMass(mass), fLifeTime(lifeTime), fCharge(charge), fMassSigma(massSigma){};
...
class KFPartEfficiencies : public TObject
{
 public:
  ...
  KFPartEfficiencies() : partDaughterPdg(0),
                         names(),
                         indices(),
                         fPdgToIndex(),
                         ratio_reco1(),
