...
KFParticlePerformanceBase::KFParticlePerformanceBase() : fParteff(), fPVeff(), fPVeffMCReconstructable(), outfileName(), histodir(0), fNEvents(0), fStoreMCHistograms(1), fStorePrimSecHistograms(1), fStoreZRHistograms(1), fHistoDir(0)
{
  ...
  }
  {
    ...
    for (int iPart = 0; iPart < fParteff.nParticles; ++iPart) {
      if (!(decays.empty()) && (iPart < fParteff.fFirstStableParticleIndex || iPart > fParteff.fLastStableParticleIndex))
        if (decays.find(fParteff.partPDG[iPart]) == decays.end())
          continue;
...
bool KFParticlePerformanceBase::IsCollectZRHistogram(int iParticle) const
{
  ...
  return (abs(fParteff.partPDG[iParticle]) == 310 ||
         ...
         fStoreMCHistograms && fStoreZRHistograms;
}
